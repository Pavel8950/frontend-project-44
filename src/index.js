...
function userInteraction (funcSource, description) {
    const name = userName(); 
    let questionAnswer = [];
    let question = 0;
    let correctAnswer = 0;
    ...
    for (let i = 0; i < 3; i += 1) {
        ...
        if ((userAnswer === correctAnswer) && funcSource === 'brainEven') { 
            console.log('Correct!');
        } else if ( Number(userAnswer) === Number(correctAnswer) && funcSource != 'brainEven')  {
            console.log('Correct!');
        } else {
